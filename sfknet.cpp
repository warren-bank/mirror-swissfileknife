...
extern char *arcExtList[];
extern void tellMemLimitInfo();
extern long quietMode();
extern bool bGlblEscape;
extern char *getHTTPUserAgent();
extern long createOutDirTree(char *pszOutFile);
...
long DiskCacheConfig::setPath(char *ppath)
{
   ...
   nlen = strlen(szPath);
   long nrem = (sizeof(szPath)-10)-nlen;
   if (nrem < nsublen)
...
void CoiMap::reset(bool bWithDiskCache)
{
   ...
   for (;pcur; pcur=pnext)
   {
      ...
      if (pcoi->refcnt() > 1) {
         if (!bGlblEscape) {
            pinf("cannot drop cache entry with %ld refs: %s (%p)\n", pcoi->refcnt(), pcoi->name(), pcoi);
         }
      } else {
   ...
   extern long (*pGlblSFKStatusCallBack)(long nMsgType, char *pmsg);
   ...
   if (bWithDiskCache)
   {
      mtklog("dcache.release alloc=%ld used=%ld",nClDAlloc,nClDUsed);
...
long CoiMap::tellByteSizeChange(Coi *pcoi, num nOldSize, num nNewSize)
{
...
long CoiMap::put(char *pkey, Coi *pcoi, const char *pTraceFrom, long nmode)
{__
   ...
   while (nClByteSize + nAddSize > nGlblMemLimit)
   {
      ...
      if (!plx) {
         bool btold = 0;
         if (!btold) { btold = 1;
            pwarn("cache overflow (%ld), try to increase -memlimit (current=%ld).\n", (long)size(), (long)(nGlblMemLimit/1000000));
         }
      }
      else 
      {
         ...
         long ndropmb = (long)(pxcoi->getUsedBytes() / 1000000);
         long ncachmb = (long)(nClByteSize / 1000000);
         long nkeylen = strlen(pxkey);
         char *pxinfo = (nkeylen <= 50) ? pxkey : pxkey + nkeylen - 50;
   
         if (!quietMode()) 
         {
            char szAddInfo[100];
            snprintf(szAddInfo, sizeof(szAddInfo)-10, "%lu files %lu mb", size(), (ulong)(nClByteSize/1000000UL));
            info.setAddInfoWidth(strlen(szAddInfo));
            info.setStatus("free", pxkey, szAddInfo, eKeepAdd);
            // pinf("cache-drop: %ld %ld %.50s\n", ndropmb, ncachmb, pxinfo);
         }
...
long md5FromString(char *psz, num &rsumhi, num &rsumlo)
{
   SFKMD5 md5;
...
#ifdef USE_DCACHE
char *Coi::cacheName(char *pnamein, char *pbuf, long nmaxbuf, long *pDirPartLen)
{__
   char *prel = pnamein;
      ...
      if (c == '.') 
      {
         ...
         for (long i=0; arcExtList[i]; i++) 
         {
            if (!mystrnicmp(psrc-1, arcExtList[i], strlen(arcExtList[i]))) 
            {
               ...
               long nextlen = strlen(arcExtList[i]);
...
long CoiMap::putDiskCache(char *pkey, Coi *pcoi, const char *pTraceFrom)
{__
   ...
   mtklog("cache: disk: write pdata=%p size=%ld", pdata, (long)nsize);
...
int CoiMap::bfindDMeta(num nsumlo,num nsumhi,long &rindex)
{__
   ...
   while (1)
   {
      ...
      if (ncmp > 0) {
         ...
         // straight match
         mtklog("%ld = indexof(%lxh) used=%lu",imid,(ulong)nsumhi,nClDAlloc);
         break; // found
      }
   }
...
long CoiMap::putDiskCache(char *pkey, Coi *pcoi)
{__
   long nfree = nClDAlloc - nClDUsed;
   if (nfree < 10) {
      // expand arrays: alloc new
      long nAllocNew = nClDAlloc + (nClDAlloc ? nClDAlloc : 100);
   ...
   nClDUsed++;
   mtklog("dcache.insert at %ld of %ld: sumlo=%s key=%s", nindex, nClDUsed,
...
{__
   ...
   mtklog("cache: loaded, size=%ld: %s",(long)nFileSize,pszDiskCacheFile);
   ...
   if (strBegins((char*)pdata, "[sfk-cache-redirect]"))
   {
      ...
      *psz2 = '\0';
      ...
      mtklog("cache: loaded redir, size=%ld: %s",(long)nFileSize,pszDiskCacheFile);
   }
...
long CoiMap::remove(char *pkey) 
{
   ...
   if (pcoi->refcnt() > 1) {
      if (!bGlblEscape) {
         pinf("cannot drop cache entry with %ld refs: %s", pcoi->refcnt(), pcoi->name());
      }
   }
   else 
   {
...
{__
   ...
   if (pcore->refcnt() > 1) {
      perr("connection in use, cannot close: %s %p %ld", pszID, pcore, pcore->refcnt());
      return;
   }
...
{__
   ...
   if (praw) {
      ...
      if (pres->refcnt() > 1) {
         perr("too many locks on connection: %s (%ld)", pBaseURL, pres->refcnt());
         return 0;
      }
      pres->incref();
      return pres;
...
long ConCache::releaseClient(HTTPClient *pcln)
{__
   mtklog("concache-release %p http", pcln);
   if (pcln->refcnt() != 2) { // should NOT happen
      perr("unexpected refcnt on client: %s %ld", pcln->getID(), pcln->refcnt());
      pcln->decref();
      return 9;
   }
   pcln->decref();
...
long ConCache::releaseClient(FTPClient *pcln)
{__
...
long ConCache::forceCacheLimit()
{__
   if (size() < 10) {
      mtklog("concache-drop not required, size=%ld", size());
      return 0;
   }
   ...
   if (pcon->refcnt() > 1) {
      perr("connection cache overflow, %ld %ld", size(), pcon->refcnt());
      return 9;
   }
   ...
   mtklog("concache-remove done %p remain %ld", pcon, size());
...
TCPCon::~TCPCon( ) 
{__
   num nElapsed = getCurrentTime() - nClStartTime;
   mtklog("  tcpcon dtr %p used %ld msec",this,(long)nElapsed);
   if (clSock != INVALID_SOCKET) {
      // close was forgotten. as tcpcon's are managed by the core,
      // this is probably never reached:
      perr("missing close on tcpcon %p hsock %lxh line %d", this, (ulong)clSock, nClTraceLine);
      rawClose();
   }
...
{__
   if (clSock != INVALID_SOCKET) {
      mtklog("con %p close socket %lxh",this,(ulong)clSock);
...
long  TCPCon::read(uchar *pBlock, ulong nLen) 
{__
   ...
   long nCursor = 0;
   while (nRemain > 0) {
      long nRead = recv(clSock, (char*)pBlock+nCursor, nRemain, 0);
      // mtklog("< %ld = recv() max %ld", nRead, nRemain);
      // mtklog("    recv'd %ld", nRead);
      if (nRead <= 0)
         break;
		countIOBytes(nRead);
      nRemain -= nRead;
...
long TCPCon::send(uchar *pBlock, ulong nLen) 
{__
   return ::send(clSock, (char*)pBlock, nLen, 0);
}

char *TCPCon::buffer(long &rbufsize)
{
   if (!pClIOBuf) {
...
char *TCPCon::readLine(char *poptbuf, ulong noptmaxbuf) 
{__
   // use which buffer?
   char *pBuf    = poptbuf;
   long  nBufMax = noptmaxbuf;
...
long TCPCon::puts(char *pline) 
{__
   long nBufMax = 0;
   char *pBuf = buffer(nBufMax);
   nBufMax -= 10; // tolerance

   long nlen = strlen(pline);
   if (nlen > nBufMax) {
   ...
   // finally, send it:
   long nsent = send((uchar*)pBuf, strlen(pBuf));
   if (nsent != strlen(pBuf)) return 9;
...
long  TCPCon::setProtocol(char *psz) {
   return 9;
}

long  TCPCon::readReply(long nMinRC, long nMaxRC) {
   return 9;
}
...
long TCPCore::nClProxyPort = -1;
bool TCPCore::bClProxySet  =  0;

// this may also be called BEFORE sysInit().
long TCPCore::setProxy(char *phost, long nport) 
{__
...
long TCPCore::incref( )  { return ++nClRefs; }
long TCPCore::decref( )  { return --nClRefs; }
long TCPCore::refcnt( )  { return nClRefs;   }

long TCPCore::lastError( ) {
   #ifdef _WIN32
...
{__
   mtklog("tcpcore closecon %p ncon=%ld", this, nClCon);
   while (nClCon > 0) {
...
long TCPCore::makeServerSocket (long nportin, TCPCon **ppout)
{__
   ...
   if (hServSock == INVALID_SOCKET)
      return 9+perr("cannot create socket on port %lu", nPort);

   if (bind(hServSock, (struct sockaddr *)&ServerAdr, sizeof(sockaddr_in)) == SOCKET_ERROR)
      return 9+perr("cannot bind socket on port %lu", nPort);
   ...
   */
   ...
   if (listen(hServSock, 4) == SOCKET_ERROR)
      return 9+perr("cannot listen on port %lu", nPort);
...
long TCPCore::addCon(TCPCon *pcon) 
{__
   if (nClCon >= FD_SETSIZE-2)
...
long TCPCore::remCon(TCPCon *pcon) 
{__
   // remove from fdset
   ...
   for (long i=nAtPos; i<nClCon-1; i++)
      aClCon[i] = aClCon[i+1];
...
long TCPCore::accept(TCPCon *pServerCon, TCPCon **ppout) 
{__
   if (checksys()) return 9;
   ...
   if (hClient == INVALID_SOCKET)
      return 9+perr("accept failed (%ld)", lastError());
...
long TCPCore::checksys( ) {
   if (bSysInitDone) return 0;
   return 9+perr("internal: missing tcp sysinit");
}

long TCPCore::connect(char *pszHost, long nportin, TCPCon **ppout)
{__
   if (checksys()) return 9;
   ...
   if ((::connect(hSock, (struct sockaddr *)&sock, sizeof(sock))) == -1) {
      ...
      return 9+perr("cannot connect to %s:%lu, rc %d\n", pszHost, nPort, nerr);
   }
...
long TCPCore::close(TCPCon *pcon) 
{__
   remCon(pcon);
   mtklog("tcpcore %p close socket %lxh",this,(ulong)pcon->clSock);
...
long  TCPCore::selectInput (TCPCon **ppNextActiveCon, TCPCon *pToQuery) 
{__
   mtklog("tcp-core select on %ld connections",nClCon);
   ...
   if (nrc <= 0)
      return 9+perr("select() failed, %ld",lastError());
   // identify connection with input
   mtklog("tcp-core select rc %ld",nrc);
   for (long i=0; i<nClCon; i++)
   {
      TCPCon *pcon = aClCon[i];
...
long  TCPCore::registerData(TCPCon *pcon, TCPConData *pUserData) {
   return 9;
}
...
{__
   if (pClCache) {
      // if (nClCacheUsed)
      //    pwarn("http: %ld unused bytes remaining in cache, %p\n",nClCacheUsed,this);
      delete [] pClCache;
   }
   if (pszClLineCache) {
...
long HTTPClient::joinURL(char *pabs, char *pref)
{__
   // 1) "image.png"
   // 1) "subdir/the.txt"
   // 2) "/topdir/the.txt"
...
long HTTPClient::splitURL(char *purl)
{__
...
long HTTPClient::splitHeader(char *phead)
{__
   // isolate hostname from url
   aClHeadBuf[0] = '\0';
   pClCurVal = 0;
...
// returns RC
long HTTPClient::sendLine(char *pline) {
   if (!pClCurCon)
      { perr("int. #175292 missing connection"); return 9; }
   return pClCurCon->puts(pline);
}

long HTTPClient::list(char *purl, CoiTable **pptable)
{
   return 9;
}

long HTTPClient::getFileHead(char *purl, Coi *pcoi, char *pinfo)
{__
   ...
   if (connectHttp(phost, nport, &pClCurCon)) {
      bClNoCon = 1;
      return 9+perr("http connect failed: %s:%ld",phost,nport);
   }
   ...
   do
   {
      ...
      char *pcont = pheads->get("content-type");
         ...
         )
      {
         ...
         close();
         if (connectHttp(phost, nport, &pClCurCon))
            return 9+perr("http reconnect failed: %s:%ld",phost,nport);
         continue;
      }
      ...
      // "Location: main.html"
      char *ptarg = pheads->get("location");
      if (!ptarg) return 9+perr("rc %ld but no Location on %s", nwebrc, curpath());
...
long HTTPClient::rawReadHeaders
 (
   long  &rwebrc,
...
long HTTPClient::connectHttp(char *phostorip, long nport, TCPCon **ppout)
{__
   if (nClProxyPort < 0)
...
long HTTPClient::open(char *purl, char *pmode, Coi *pcoi)
{__
   if (bClNoCon) {
...
_     // send request to return headers.
      long nrc = sendReq("GET",pfile,phost,nport);
      if (nrc) return nrc;
_
      // read headers on reply stream.
      long nwebrc = 0;
...
long HTTPClient::readraw(uchar *pbuf, long nbufsize) 
{__
   if (!pClCurCon)
...
long HTTPClient::read(uchar *pbuf, long nbufmax) 
{__
   mtklog("http::read max=%ld chunked=%d incache=%ld", nbufmax, bClChunked, nClCacheUsed);
   ...
   if (nClCacheUsed <= 0)
   {
      ...
      long nchunksize = strtol(pline, 0, 0x10);
      mtklog("http next chunk %ld bytes \"%s\"", nchunksize, pline);
      if (!nchunksize)
      ...
      long nCacheRemain = nClCacheAlloc - nClCacheUsed;
      if (nchunksize > nCacheRemain) 
      {
         // expand cache
         long nMissing  = nchunksize - nCacheRemain;
         long nAllocNew = nClCacheAlloc + nMissing + 100;
         ...
         mtklog("http cache resized to %ld", nClCacheAlloc);
      ...
      long nread  = pClCurCon->read(pdst, nchunksize);
      mtklog("http read %ld bytes", nread);
      if (nread < nchunksize)
         return 0+perr("http: incomplete chunk read, %ld %ld",nread,nchunksize);
   ...
   long   nrem = nClCacheUsed - ntocopy;
   memmove(pClCache, psrc, nrem);
   nClCacheUsed -= ntocopy;

   mtklog("http cache remain %ld alloc %ld", nClCacheUsed, nClCacheAlloc);
...
{__
   mtklog("http::close: pcon=%p cacheAlloc=%ld cacheUsed=%ld", pClCurCon, nClCacheAlloc, nClCacheUsed);
...
long   FTPClient::lineMax( ) { return sizeof(szClLineBuf)-100; }
...
long FTPClient::curport( ) { return nClPort; }

long FTPClient::splitURL(char *purl)
{__
   // isolate hostname from url
   aClURLBuf[0] = '\0';
...
long FTPClient::sendLine(char *pline) {
   return pClCtlCon->puts(pline);
}

// guarantees to set pline, even if empty
long FTPClient::readReply(char **ppline)
{__
   char *pline = 0;
   *ppline = ""; // in any case
   long ncode = 0;
   while (pline = readLine()) {
      ...
      mtklog("[reply %ld continued]",ncode);
   }
   *ppline = pline ? pline : (char*)"";
   return pline ? ncode : 0;
}

long FTPClient::login(char *phostorip, long nport)
{__
   ...
   if (connect(phostorip, nport, &pClCtlCon))
      return 9+perr("ftp login failed: %s:%ld",phostorip,nport);
...
_
   if (sendLine("USER anonymous")) return 9;
   if (!readLine()) return 9; // 331
   if (sendLine("PASS sft102@")) return 9;
_
   // expect "230 login done"
   if (!(ncode = readReply(&pline)))
      return 9+perr("ftp: wrong login reply: %s", pline);
_
   if (sendLine("TYPE I")) return 9;
...
long FTPClient::loginOnDemand(char *phostorip, long nport)
{__
...
long FTPClient::list(char *pdir, CoiTable **ppout, char *pRootURL)
{__
   if (!pdir) return 9;

   if (!*pdir)    pdir = ".";
   if (!pRootURL) pRootURL = ""; // safety
   ...
   if (bClSFT) {
      ...
      if (nrc = setPassive(&pdat))
         return 9+perr("set passive failed, %ld", nrc);
      pdat->setBlocking(0);
   ...
   while (1)
   {
      ...
      if (bClSFT) {
         ...
         TCPCon *prec = 0;
         long nrc = selectInput(&prec, pdat);
   
         if (nrc > 0) {
            // no: check control connection
            nrc = selectInput(&prec, pClCtlCon);
            if (nrc >= 5)
               return 9+perr("ftp list failed: %ld", nrc);
   ...
   *ppout = ptab;
   ...
   mtklog("ftp list end: %s return %ld entries",pdir,ptab->numberOfEntries());

   return 0;
}

long FTPClient::readLine(TCPCon *pcon)
{__
   char *pszLineBuf  = line();
   long  nMaxLineBuf = lineMax();
   ...
   if (strBegins(pszLineBuf, "SKIP "))
   {
      ...
      ulong nLen = (ulong)atol(pszLineBuf+5);
      if (nLen > nMaxLineBuf) return 9;
      if (nLen) {
...
long FTPClient::readLong(TCPCon *pcon, ulong &rOut, char *pszInfo)
{__
   uchar szBuf[100];
   if (pcon->read(szBuf, 4) != 4) return 9+perr("failed to read %s\n", pszInfo);
   ulong nLen =   (((ulong)szBuf[3])<<24)
                | (((ulong)szBuf[2])<<16)
                | (((ulong)szBuf[1])<< 8)
                | (((ulong)szBuf[0])<< 0);
   rOut = nLen;
...
long FTPClient::sendLong(TCPCon *pcon, ulong nOut, char *pszInfo)
{__
   uchar szBuf[100];
   ...
   long nSent = pcon->send(szBuf, 4);
   if (nSent != 4) return 9+perr("failed to send %s\n", pszInfo);
   return 0;
}

long FTPClient::readNum(TCPCon *pcon, num &rOut, char *pszInfo)
{__
   uchar szBuf[100];
...
long FTPClient::readNum(uchar *pbuf, long &roff, num &rOut, char *pszInfo)
{__
   num nOut = 0;
   for (long i=0; i<8; i++) {
      nOut <<= 8;
...
long FTPClient::sendNum(TCPCon *pcon, num nOut, char *pszInfo)
{__
   uchar szBuf[100];
   ...
   long nSent = pcon->send(szBuf, 8);
   if (nSent != 8) return 9+perr("failed to send %s\n", pszInfo);
   return 0;
}

extern long nGlblTCPMaxSizeMB;
extern char *localPath(char *pAbsFile);
extern long fastMode();
...
long FTPClient::openFile(char *pfilename, char *pmode)
{__
   ...
   if (bClSFT)
   {
      ...
      if (pcon->read(abHead, nMetaSize) != nMetaSize) return 9;
...
long FTPClient::readFile(uchar *pbuf, long nmaxlen)
{__
   #ifdef USE_SFT
   if (bClSFT)
   {
      ...
      if (nmaxlen <= 0) {
         mtklog("ftp: readFile: max=%ld return=%ld", nmaxin, nmaxlen);
         return nmaxlen;
      }
      ...
      if (nClSFTVer >= 102 && file.blockmode) 
      {
         ...
         if (file.block <= nbiglatch) {
            mtklog("ftp: request %ld remain=%s", nreqlen, numtoa(file.remain));
      ...
      long nread = pClCtlCon->read(pbuf, nmaxlen);

      mtklog("ftp: read %ld", nread);
      ...
      mtklog("ftp: readFile: max=%ld return=%ld", nmaxin, nread);
...
{__
   ...
   if (bClSFT)
   {
      ...
      mtklog("ftp: closeFile %s remain=%ld", file.name ? file.name : "", (long)file.remain);
      ...
      if (nClSFTVer >= 102) {
         if (file.blockmode) {
            ...
            if (file.remain > 0) {
               pwarn("bulk transfer but %ld remaining for: %s\n", (long)file.remain, file.name ? file.name : "");
            }
            while (file.remain > 0) {
               long nread = readFile((uchar*)line(), lineMax());
               if (nread <= 0) break;
            }
         }
      ...
      if (!file.remain && file.md5) 
      {
         ...
         if (memcmp(abdig, file.abmd5, 16)) {
            pcon->send((uchar*)"EE\n\n", 4);
            perr("md5 mismatch - transfered file corrupted (size=%ld)\n", (long)file.size);
            return ;
         }
      }

      // send short confirmation, so peer can safely close socket.
      long nSent = pcon->send((uchar*)"OK\n\n", 4);
      if (nSent != 4) {
         perr("failed to send reply, %ld\n", nSent);
         return;
      }
   }
   else
...
long HTTPClient::loadFile(char *purl, num nMaxSize, uchar **ppout, num &routlen)
{__
   ...
   if (ocoi.hasSize()) {
      ...
      mtklog("read target size %ld",(long)nTargetSize);
   }
   ...
   mtklog("read-start alloc %ld used %ld",(long)nAllocSize,(long)nUsed);
   ...
   while (1) 
   {
      ...
      if (bClChunked) 
      {
         ...
         long nchunksize = strtol(pline, 0, 0x10);
         mtklog("read-chunk clen %ld \"%s\"", nchunksize, pline);
         if (!nchunksize) {
            if (strcmp(pline, "0"))
         ...
         if (nchunksize > nRemain) 
         {
            ...
            mtklog("read-chunk alloc %ld used %ld",(long)nAllocSize,(long)nUsed);
         }
      ...
      if (nRemain < 20000)
      {
         ...
         pdata = pnew;
         nAllocSize = nAllocNew;
         mtklog("read-cont  alloc %ld used %ld",(long)nAllocSize,(long)nUsed);
      }
      ...
      mtklog("%ld (%lx) = readraw(max %ld)",(long)nRead,(long)nRead,(long)nRemain);
      ...
      if (nTargetSize > 0 && nUsed >= nTargetSize) {
         if (nUsed > nTargetSize) {
            delete [] pdata;
            return 9+perr("download has unexpected size: %s %ld/%ld", purl, (long)nUsed, (long)nTargetSize);
         }
   ...
   if (nTargetSize > 0 && nUsed < nTargetSize) {
      delete [] pdata;
      return 9+perr("download incomplete: %s %ld/%ld", purl, (long)nUsed, (long)nTargetSize);
   }
   ...
   *ppout  = pdata;
   ...
   mtklog("%s downloaded in %ld msec", purl, (long)nElapsed);

   return 0;
}

long CoiData::getFtp(char *purl)
{__
   TCPCore::sysInit();

   if (pClFtp) return 0; // reuse

   long nprelen = strlen("ftp://");
   if (strlen(purl) <= nprelen) return 9;
...
long CoiData::getHttp(char *purl)
{__
   TCPCore::sysInit();

   if (pClHttp) return 0; // reuse

   long nprelen = strlen("http://");
   if (strlen(purl) <= nprelen) return 9;
...
char *Coi::header(char *pname) {
   if (!hasData()) return 0;
   if (!data().pClHeaders) return 0;
   return data().headers().get(pname);
}

bool sameDomain(char *psz1in, char *psz2in, long &rdomlen)
{
   char *psz1 = psz1in;
   char *psz2 = psz2in;
   ...
   long nlen1 = psla1 - psz1in;
   long nlen2 = psla2 - psz2in;
...
{__
   if (data().bloaddirdone) {
      mtklog("coi::loaddir: done, %ld entries", data().elements().numberOfEntries());
...
{__
   ...
   mtklog(" ... %ld bytes page",(long)strlen(ppage));
   ...
   for (; psz1 && *psz1; psz1 = pszc)
   {
      ...
      *psz2++ = '\0';
      ...
      if (strBegins(pref, "http://")) {
         ...
         mtklog("%d = sameDomain(%s, %s) %ld",brc,name(),pref,ndomlen);
         if (!brc)
   ...
   /*
      if (apBinExtList[iext]) {
         ...
         mtklog("   added binfile \"%s\" offs %ld", pabsref, (long)(psz1-ppage));
         continue;
      }
   */
   ...
   /* 
      ...
      else 
      {
         ...
         ocoi.setIsDir(1);
         data().elements().addEntry(ocoi); // is copied
         mtklog("   added dir  \"%s\" offs %ld", pabsref, (long)(psz1-ppage));

         // 2. AND create a file entry with the same name
         ocoi.setIsDir(0);
         data().elements().addEntry(ocoi); // is copied
         mtklog("   added file \"%s\" offs %ld", pabsref, (long)(psz1-ppage));
      }
   */
...
{__
   ...
   if (pcached) 
   {
      bool bhasptr = pcached->data().src.data ? 1 : 0;
      mtklog("coi::nexthttpentry cache hit %d %ld %s", (int)bhasptr, (long)pcached->data().src.size, pcached->name());
      return pcached;
   }
   else 
   {
...
Coi *Coi::rawNextFtpEntry( ) 
{
   ...
   {
      bool bhasptr = pcached->data().src.data ? 1 : 0;
      mtklog("coi::nextftpentry cache hit %d %ld %s", (int)bhasptr, (long)pcached->data().src.size, pcached->name());
      return pcached;
   }
   else 
   {
...
long Coi::rawOpenFtpSubFile(char *pmode) 
{
   if (!isFtp()) return 9;
...
long Coi::rawOpenHttpSubFile(char *pmode)
{__
   if (!isHttp()) return 9;
   if (strcmp(pmode, "rb")) return 9;
   ...
   long nrc = phttp->open(name(), pmode, this);
...
size_t Coi::rawReadFtpSubFile(void *pbufin, size_t nBufSize) 
{
   ...
   long nread = data().pClFtp->readFile((uchar*)pbufin, (long)nBufSize);
   mtklog(" ftp read %s %lu done %ld", name(), (ulong)nBufSize, nread);
...
{__
   ...
   long nread = data().pClHttp->read((uchar*)pbufin, (long)nBufSize);
   mtklog("http-read %s done=%ld max=%ld", name(), nread, (long)nBufSize);
...
long Coi::prefetch(bool bLoadNonArcBinaries, num nFogSizeLimit, num nHardSizeLimit)
{
   ...
   {
      char *pctype = header("content-type"); // or null
...
long Coi::loadOwnFileRaw(num nmaxsize, uchar **ppout, num &rsize)
{
...
long Coi::provideInput(int nTraceLine, bool bsilent)
{__
...
_     return 0; // nothing to do
   ...
   if (nSize > nGlblMemLimit) {
      snprintf(data().szlasterr, sizeof(data().szlasterr)-4,
         " - content too large (%lu mb)", (ulong)(nClSize/1000000UL));
   ...
   mtklog("cload ok, %ld bytes: %s", (long)nsize, name());
...
{__
   ...
   Coi *psub = 0;
   for (long i=0; i<data().elements().numberOfEntries(); i++)
   {
