...
{
   if (argc < 3) {
      printf("usage: instr file.cpp includefile macroname\n"
             "       instr file.cpp -revoke\n"
             "e.g.   instr mysrc.cpp mydebmacros.hpp _sb_\n"
            );
      return 0;
   }
   ...
   pszGlblInclude = argv[idir++];
   pszGlblMacro   = argv[idir++];

   while (idir < argc) {
      char *pszopt = argv[idir++];
      if (!strcmp(pszopt, "-quiet"))
         bGlblQuiet = 1;
      else
      if (!strcmp(pszopt, "-debug"))
         bdebug = 1;
      else {
   ...
   SrcParse *pparse = new SrcParse();
   ulong nHits = pparse->processFile(pFile, 0, fout);
...
/* example for central include:

#ifndef _INSTRMAIN_DEFINED_
#define _INSTRMAIN_DEFINED_

class InstrMain {
public:
   InstrMain(const char *pszfile, int nline) {
      mfile = pszfile;
      mline = nline;
   }
   ~InstrMain() {
...
#define _sb_ {InstrMain tmp983451(__FILE__,__LINE__); tmp983451.dummy();}
